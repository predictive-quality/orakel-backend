...
def validate_and_return_id_token(jws, nonce=None, validate_nonce=True):
    ...
    shared_key = CLIENT_SECRET \
        if SIGNATURE_ALG == 'HS256' \
        else SIGNATURE_KEY  # RS256
...
def _get_jwks_keys(shared_key):
    ...
    jwks_keys = KEYS()
    jwks_keys.load_from_url(JWKS_ENDPOINT)
...
def _validate_claims(id_token, nonce=None, validate_nonce=True):
    ...
    provider_parsed_url = urlparse(PROVIDER_URI)
    ...
    if utc_timestamp > id_token['iat'] + ID_TOKEN_MAX_AGE:
    ...
    if validate_nonce and USE_NONCE and id_token_nonce != nonce:
